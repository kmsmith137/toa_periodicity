...
def make_daniele_histogram(ax, data_filename, maxgap, smin=2.0, smax=14.0):
    data_dir = 'data_v2'
    nevents = 10**6
    chi = 0.2
    ...
    sfilename = sim_filename(chi, npulses, maxgap, nevents)
    print(f'Reading {sfilename}')
    sims = np.load(sfilename)
    assert sims.shape == (nevents,)

    ax.set_xlabel('Score $\hat S$')
    ax.set_ylabel('Counts')
    ax.hist(sims, bins=np.linspace(0.0,smax,100), log=True, color='grey')
    ax.axvline(data_score, color='black', ls=':')


####################################################################################################


def read_pulsar_toas():
    filename = 'pulsar_data/B1919+21_59200_sp.tim'
    ret = [ ]
